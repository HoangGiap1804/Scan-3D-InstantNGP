...
class NeRFGUI:
    ...
    def train_step(self):
        ...
        if 'psnr' in outputs:
            dpg.set_value("_log_train_psnr", f'{outputs["psnr"]:.4f} dB')
        else:
            dpg.set_value("_log_train_psnr", "no data")

        psnr_str = f', psnr = {outputs["psnr"]:.2f}dB' if 'psnr' in outputs else ''
        dpg.set_value("_log_train_log", f'step = {self.step: 5d} (+{self.train_steps: 2d}), loss = {outputs["loss"]:.4f}{psnr_str}, lr = {outputs["lr"]:.5f}')
    ...
    def register_dpg(self):
        ...
        with dpg.window(label="Control", tag="_control_window", width=400, height=300):
            ...
            if not self.opt.test:
                ...
                with dpg.group(horizontal=True):
                    dpg.add_text("Train PSNR: ")
                    dpg.add_text("no data", tag="_log_train_psnr")
